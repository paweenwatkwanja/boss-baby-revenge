...
# instructions
1. choices of running a program
    1. run with input from the terminal
        - This is for a short string of sequence with less than a thousand characters

    2. run with input from *.txt file for a very long sequence
...
1. time complexity
    - O(1)
        ...
        Therefore, I use slicing method to achieve these and return right after either of these conditions is met. This results in O(1).
    - O(n)
        ...
        That's why the solution is to traverse through the sequence. Using loop uses computation resources and I do not want to waste it. Therefore, I come up with an idea of evaluating the conditions while traversing is in process. 

        Therefore, the time complexity is O(n) for both average and worst case.
...
2. space complexity
    ...
    - O(1)
        This complexity is for logic while processing the input. As it uses simple data structure such as a variable with an int data type for a counter, that does not use many resources for each character of the input. The resources do not increase if the input size changes. That's why its space complexity is O(1)
